<?php
  ...
  if (isset($_POST['Sales'])){
    ...
    if($_POST['sales_id'] == 0){
      ...
      $sql = "INSERT INTO sales(purchase_id, stock_id, sales_date,sales_time,stock_name,quantity,consumable,delivered_from,delivered_to,cost,report)
            values('$Purchase_id','$stock_id','$date','$time','$stock_Name','$Quantity','$Consumable','$deliveredfrom','$deliveredto','$Cost','$Report')";
      if(mysqli_query($db, $sql)){
        ...
        $qry = mysqli_query($db, $sq);
        $row = $qry -> fetch_assoc();
        if($Purchase_id != $row['purchase_id'] && $stock_id != $row['stock_id']){
          ...
          $in = "INSERT INTO stock_details(purchase_id, stock_id, stock_name, quantity, dept_residing, consumable, report, sales_id) VALUES('$Purchase_id','$stock_id','$stock_Name','$Qty','$deliveredfrom','$Consumable','$Report','$las_id')";
          if(mysqli_query($db, $in)){
            header('location: sales.php');
          }
          else{
            echo "ERROR: Could not able to execute $in. " . mysqli_error($db);
          }
        }
        header('location: sales.php');  
      } 
      ...
      if(mysqli_query($db, $sql)){
        $ver = "SELECT quantity from stock_details where stock_id = '$stock_id'";
         ...
         $qty = $row['quantity'];
         $Qty = $qty - $Quantity;
         $up = "UPDATE stock_details SET quantity = '$Qty', sales_id = '$sales_id' where stock_id = '$stock_id'";
        if(mysqli_query($db, $up)){
